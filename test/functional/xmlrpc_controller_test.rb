...
class XmlrpcControllerApiTest < Test::Rails::TestCase
  ...
  def test_should_get_users_blogs
    ...
    assert_equal "WRS Blog", blogs[0]['blogName']
  end
  ...
  def test_get_post
    post = invoke_layered :metaWeblog, :getPost, blogs(:one).id.to_s, 'isaac@example.com', 'test'
    assert_equal blogs(:one).id.to_s, post['postId']
    assert_equal blogs(:one).title, post['title']
    assert_equal blogs(:one).body, post['description']
  end
  
  def test_should_not_get_post_without_login
    post = invoke_layered :metaWeblog, :getPost, blogs(:one).id.to_s, 'isaac@example.com', 'NOTMYPASSWORD'
    assert_nil post
    post = invoke_layered :metaWeblog, :getPost, blogs(:one).id.to_s, 'NOT_A_REAL_LOGIN', 'test'
    assert_nil post
  end  
  ...
  def test_get_recent_posts
    ...
    post = Blog.find(first_post['postId'])
