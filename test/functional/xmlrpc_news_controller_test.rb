...
require 'xmlrpc_news_controller'

class XmlrpcNewsController; def rescue_action(e) raise e end; end

class XmlrpcNewsControllerApiTest < Test::Rails::TestCase
  fixtures :entries, :users
  
  def setup
    @controller = XmlrpcNewsController.new
  ...
  def test_should_get_users_blogs
    ...
    assert_equal "WRS News", blogs[0]['blogName']
  end
  ...
  def test_get_post
    post = invoke_layered :metaWeblog, :getPost, entries(:one).id.to_s, 'isaac@example.com', 'test'
    assert_equal entries(:one).id.to_s, post['postId']
    assert_equal entries(:one).title, post['title']
    assert_equal entries(:one).body, post['description']
  end
  
  def test_should_not_get_post_without_login
    post = invoke_layered :metaWeblog, :getPost, entries(:one).id.to_s, 'isaac@example.com', 'NOTMYPASSWORD'
    assert_nil post
    post = invoke_layered :metaWeblog, :getPost, entries(:one).id.to_s, 'NOT_A_REAL_LOGIN', 'test'
    assert_nil post
  end  
  ...
  def test_get_recent_posts
    ...
    post = Entry.find(first_post['postId'])
