...
class UserTest < ActiveSupport::TestCase
  ...
  def test_should_create_user
    assert_difference 'User.count' do
      user = create_user
      #assert false, "USER: #{user.inspect}"
      assert !user.new_record?, "ERROR: #{user.errors.full_messages.to_sentence}"
    end
  end 
  
=begin
  def test_login_too_short
  ...
  def test_login_too_long
    assert_no_difference 'User.count' do
      u = create_user(
        :login => "abcdefghijabcdefghijabcdefghijabcdefghiji") #41 chars long
      assert u.errors.on(:login)
    end
  end 
  ...
  def test_name_too_long
    assert_no_difference 'User.count' do
      u = create_user(
        :name => "abcdefghijabcdefghijabcdefghijabcdefghiji") #41 chars long
      assert u.errors.on(:name)
    end
  end
=end
  ...
  def test_password_too_long
    assert_no_difference 'User.count' do
      u = create_user(
        :password => "abcdefghijabcdefghijabcdefghijabcdefghiji",        
        :password_confirmation => "abcdefghijabcdefghijabcdefghijabcdefghiji") #41 chars long
      assert u.errors.on(:password)
    end
  end 
=begin 
...
=end
  ...
  def test_should_require_valid_email
    assert_no_difference 'User.count' do
      u = create_user(:email => "notanemail_at_yahoo")
      assert u.errors.on(:email)
    end
  end
