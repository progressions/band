class Mailer < ActionMailer::Base  
  ...
  def signup_confirmation fan
    ...
    body        :fan => fan, 
                ...
                :title => "Thank you for joining the #{global_settings.artist_name} mailing list!",
  ...
  def signup_report fan
    ...
    body        :fan => fan, 
                :global_settings => global_settings, 
                :title => "#{fan.email} has joined the mailing list!",
                :shows => Show.find(:all, :limit => 3, :order => 'date ASC', :conditions => ["date > ?", Time.now]), 
                :blogs => Blog.active.posted_yet.find(:all, :limit => 3, :order => "posted_at DESC, created_at DESC")
    content_type "text/html"
  end
  ...
  def comment_notification user, blog, comment
    ...
    body        :user => user, 
                ...
                :comment => comment, 
                :global_settings => global_settings, 
                :title => "#{comment.name} has commented on your blog posting '#{blog.title}'",
                :shows => Show.find(:all, :limit => 3, :order => 'date ASC', :conditions => ["date > ?", Time.now]), 
                :blogs => Blog.active.posted_yet.find(:all, :limit => 3, :order => "posted_at DESC, created_at DESC")
    content_type "text/html"
  end
  ...
  def comment_subscription email, blog, comment
    ...
    body        :email => email, 
                ...
                :comment => comment, 
                :global_settings => global_settings, 
                :title => "#{comment.name} has commented on the blog posting '#{blog.title}'"
    content_type "text/html",
                :shows => Show.find(:all, :limit => 3, :order => 'date ASC', :conditions => ["date > ?", Time.now]), 
                :blogs => Blog.active.posted_yet.find(:all, :limit => 3, :order => "posted_at DESC, created_at DESC")
  end
