class Mailer < ActionMailer::Base  
  ...
  def mail fan, mail
    ...
    body        :fan => fan, :mail => mail, :global_settings => global_settings, :shows => Show.find(:all, :limit => 3, :order => 'date ASC', :conditions => ["date > ?", Time.now]), :blogs => Blog.active.posted_yet.find(:all, :limit => 3, :order => "posted_at DESC, created_at DESC"), :title => mail.title
    content_type "text/html"
  end
